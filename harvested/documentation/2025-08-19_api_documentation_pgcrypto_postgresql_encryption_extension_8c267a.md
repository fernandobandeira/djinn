...
confidence: 0.95
harvested_at: '2025-08-19T18:48:10.563347'
profile: api_documentation
source: https://www.postgresql.org/docs/current/pgcrypto.html
topic: pgcrypto PostgreSQL Encryption Extension
---

# pgcrypto PostgreSQL Encryption Extension
...
Supported Versions: [Current](https://www.postgresql.org/docs/current/pgcrypto.html "PostgreSQL 17 - F.26. pgcrypto — cryptographic functions") ([17](https://www.postgresql.org/docs/17/pgcrypto.html "PostgreSQL 17 - F.26. pgcrypto — cryptographic functions")) / [16](https://www.postgresql.org/docs/16/pgcrypto.html "PostgreSQL 16 - F.26. pgcrypto — cryptographic functions") / [15](https://www.postgresql.org/docs/15/pgcrypto.html "PostgreSQL 15 - F.26. pgcrypto — cryptographic functions") / [14](https://www.postgresql.org/docs/14/pgcrypto.html "PostgreSQL 14 - F.26. pgcrypto — cryptographic functions") / [13](https://www.postgresql.org/docs/13/pgcrypto.html "PostgreSQL 13 - F.26. pgcrypto — cryptographic functions")
Development Versions: [18](https://www.postgresql.org/docs/18/pgcrypto.html "PostgreSQL 18 - F.26. pgcrypto — cryptographic functions") / [devel](https://www.postgresql.org/docs/devel/pgcrypto.html "PostgreSQL devel - F.26. pgcrypto — cryptographic functions")
Unsupported versions: [12](https://www.postgresql.org/docs/12/pgcrypto.html "PostgreSQL 12 - F.26. pgcrypto — cryptographic functions") / [11](https://www.postgresql.org/docs/11/pgcrypto.html "PostgreSQL 11 - F.26. pgcrypto — cryptographic functions") / [10](https://www.postgresql.org/docs/10/pgcrypto.html "PostgreSQL 10 - F.26. pgcrypto — cryptographic functions") / [9.6](https://www.postgresql.org/docs/9.6/pgcrypto.html "PostgreSQL 9.6 - F.26. pgcrypto — cryptographic functions") / [9.5](https://www.postgresql.org/docs/9.5/pgcrypto.html "PostgreSQL 9.5 - F.26. pgcrypto — cryptographic functions") / [9.4](https://www.postgresql.org/docs/9.4/pgcrypto.html "PostgreSQL 9.4 - F.26. pgcrypto — cryptographic functions") / [9.3](https://www.postgresql.org/docs/9.3/pgcrypto.html "PostgreSQL 9.3 - F.26. pgcrypto — cryptographic functions") / [9.2](https://www.postgresql.org/docs/9.2/pgcrypto.html "PostgreSQL 9.2 - F.26. pgcrypto — cryptographic functions") / [9.1](https://www.postgresql.org/docs/9.1/pgcrypto.html "PostgreSQL 9.1 - F.26. pgcrypto — cryptographic functions") / [9.0](https://www.postgresql.org/docs/9.0/pgcrypto.html "PostgreSQL 9.0 - F.26. pgcrypto — cryptographic functions") / [8.4](https://www.postgresql.org/docs/8.4/pgcrypto.html "PostgreSQL 8.4 - F.26. pgcrypto — cryptographic functions") / [8.3](https://www.postgresql.org/docs/8.3/pgcrypto.html "PostgreSQL 8.3 - F.26. pgcrypto — cryptographic functions")
F.26. pgcrypto — cryptographic functions  
---  
[Prev](https://www.postgresql.org/docs/current/pgbuffercache.html "F.25. pg_buffercache — inspect PostgreSQL buffer cache state") | [Up](https://www.postgresql.org/docs/current/contrib.html "Appendix F. Additional Supplied Modules and Extensions") | Appendix F. Additional Supplied Modules and Extensions | [Home](https://www.postgresql.org/docs/current/index.html "PostgreSQL 17.6 Documentation") |  [Next](https://www.postgresql.org/docs/current/pgfreespacemap.html "F.27. pg_freespacemap — examine the free space map")  
## F.26. pgcrypto — cryptographic functions [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO) 

[F.26.1. General Hashing Functions](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-GENERAL-HASHING-FUNCS)


[F.26.2. Password Hashing Functions](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PASSWORD-HASHING-FUNCS)


[F.26.3. PGP Encryption Functions](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS)


[F.26.4. Raw Encryption Functions](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-RAW-ENC-FUNCS)


[F.26.5. Random-Data Functions](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-RANDOM-DATA-FUNCS)


[F.26.6. Notes](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-NOTES)


[F.26.7. Author](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-AUTHOR)
...
### F.26.1. General Hashing Functions [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-GENERAL-HASHING-FUNCS)
#### F.26.1.1. `digest()` [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-GENERAL-HASHING-FUNCS-DIGEST)
```
...
Computes a binary hash of the given _`data`_._`type`_is the algorithm to use. Standard algorithms are`md5` , `sha1`, `sha224`, `sha256`, `sha384` and `sha512`. Moreover, any digest algorithm OpenSSL supports is automatically picked up.
...
#### F.26.1.2. `hmac()` [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-GENERAL-HASHING-FUNCS-HMAC)
```
...
Calculates hashed MAC for _`data`_with key _`key`_._`type`_is the same as in`digest()`.
...
[Table F.17](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-CRYPT-ALGORITHMS "Table F.17. Supported Algorithms for crypt\(\)") lists the algorithms supported by the `crypt()` function.
...
#### F.26.2.1. `crypt()` [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PASSWORD-HASHING-FUNCS-CRYPT)
```
crypt(password text, salt text) returns text

```

Calculates a crypt(3)-style hash of _`password`_. When storing a new password, you need to use`gen_salt()` to generate a new _`salt`_value. To check a password, pass the stored hash value as _`salt`_, and test whether the result matches the stored value.
...
#### F.26.2.2. `gen_salt()` [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PASSWORD-HASHING-FUNCS-GEN-SALT)
```
...
The _`type`_parameter specifies the hashing algorithm. The accepted types are:`des` , `xdes`, `md5` and `bf`.
The _`iter_count`_parameter lets the user specify the iteration count, for algorithms that have one. The higher the count, the more time it takes to hash the password and therefore the more time to break it. Although with too high a count the time to calculate a hash may be several years — which is somewhat impractical. If the _`iter_count`_parameter is omitted, the default iteration count is used. Allowed values for _`iter_count`_depend on the algorithm and are shown in[Table F.18](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-ICFC-TABLE "Table F.18. Iteration Counts for crypt\(\)").
**Table F.18. Iteration Counts for`crypt()`**
...
[Table F.19](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-HASH-SPEED-TABLE "Table F.19. Hash Algorithm Speeds") gives an overview of the relative slowness of different hashing algorithms. The table shows how much time it would take to try all combinations of characters in an 8-character password, assuming that the password contains either only lower case letters, or upper- and lower-case letters and numbers. In the `crypt-bf` entries, the number after a slash is the _`iter_count`_parameter of`gen_salt`.
**Table F.19. Hash Algorithm Speeds**
...
#### F.26.3.1. `pgp_sym_encrypt()` [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-PGP-SYM-ENCRYPT)
```
...
Encrypt _`data`_with a symmetric PGP key _`psw`_. The _`options`_parameter can contain option settings, as described below.
#### F.26.3.2. `pgp_sym_decrypt()` [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-PGP-SYM-DECRYPT)
```
...
The _`options`_parameter can contain option settings, as described below.
#### F.26.3.3. `pgp_pub_encrypt()` [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-PGP-PUB-ENCRYPT)
```
...
Encrypt _`data`_with a public PGP key _`key`_. Giving this function a secret key will produce an error.
The _`options`_parameter can contain option settings, as described below.
#### F.26.3.4. `pgp_pub_decrypt()` [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-PGP-PUB-DECRYPT)
```
...
Decrypt a public-key-encrypted message. _`key`_must be the secret key corresponding to the public key that was used to encrypt. If the secret key is password-protected, you must give the password in _`psw`_. If there is no password, but you want to specify options, you need to give an empty password.
...
The _`options`_parameter can contain option settings, as described below.
#### F.26.3.5. `pgp_key_id()` [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-PGP-KEY-ID)
```
...
#### F.26.3.6. `armor()`, `dearmor()` [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-ARMOR)
```
...
If the _`keys`_and _`values`_arrays are specified, an _armor header_ is added to the armored format for each key/value pair. Both arrays must be single-dimensional, and they must be of the same length. The keys and values cannot contain any non-ASCII characters.
#### F.26.3.7. `pgp_armor_headers` [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-PGP-ARMOR-HEADERS)
```
...
#### F.26.3.8. Options for PGP Functions [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-OPTS)
...
##### F.26.3.8.1. cipher-algo [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-OPTS-CIPHER-ALGO)
Which cipher algorithm to use.
Values: bf, aes128, aes192, aes256, 3des, cast5 Default: aes128 Applies to: pgp_sym_encrypt, pgp_pub_encrypt
##### F.26.3.8.2. compress-algo [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-OPTS-COMPRESS-ALGO)
...
##### F.26.3.8.3. compress-level [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-OPTS-COMPRESS-LEVEL)
...
##### F.26.3.8.4. convert-crlf [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-OPTS-CONVERT-CRLF)
...
##### F.26.3.8.5. disable-mdc [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-OPTS-DISABLE-MDC)
...
##### F.26.3.8.6. sess-key [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-OPTS-SESS-KEY)
...
##### F.26.3.8.7. s2k-mode [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-OPTS-S2K-MODE)
...
##### F.26.3.8.8. s2k-count [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-OPTS-S2K-COUNT)
...
##### F.26.3.8.9. s2k-digest-algo [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-OPTS-S2K-DIGEST-ALGO)
Which digest algorithm to use in S2K calculation.
Values: md5, sha1 Default: sha1 Applies to: pgp_sym_encrypt
##### F.26.3.8.10. s2k-cipher-algo [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-OPTS-S2K-CIPHER-ALGO)
Which cipher to use for encrypting separate session key.
Values: bf, aes, aes128, aes192, aes256 Default: use cipher-algo Applies to: pgp_sym_encrypt
##### F.26.3.8.11. unicode-mode [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-OPTS-UNICODE-MODE)
...
Values: 0, 1 Default: 0 Applies to: pgp_sym_encrypt, pgp_pub_encrypt
#### F.26.3.9. Generating PGP Keys with GnuPG [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-GNUPG)
...
#### F.26.3.10. Limitations of PGP Code [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-PGP-ENC-FUNCS-LIMITATIONS)
...
Encrypt/decrypt data using the cipher method specified by _`type`_. The syntax of the _`type`_string is:
```
_algorithm_ [ - _mode_ ] [ /pad: _padding_ ]

```

where _`algorithm`_is one of:
  * `bf` — Blowfish
  * `aes` — AES (Rijndael-128, -192 or -256)


and _`mode`_is one of:
  * `cbc` — next block depends on previous (default)
  * `ecb` — each block is encrypted separately (for testing only)


and _`padding`_is one of:
...
In `encrypt_iv` and `decrypt_iv`, the _`iv`_parameter is the initial value for the CBC mode; it is ignored for ECB. It is clipped or padded with zeroes if not exactly block size. It defaults to all zeroes in the functions without this parameter.
### F.26.5. Random-Data Functions [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-RANDOM-DATA-FUNCS)
```
...
Returns _`count`_cryptographically strong random bytes. At most 1024 bytes can be extracted at a time. This is to avoid draining the randomness generator pool.
```
...
### F.26.6. Notes [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-NOTES)
#### F.26.6.1. Configuration [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-NOTES-CONFIG)
...
#### F.26.6.2. NULL Handling [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-NOTES-NULL-HANDLING)
...
#### F.26.6.3. Security Limitations [#](https://www.postgresql.org/docs/current/pgcrypto.html#PGCRYPTO-NOTES-SEC-LIMITS)
...
---|---|---  
F.25. pg_buffercache — inspect PostgreSQL buffer cache state  | [Home](https://www.postgresql.org/docs/current/index.html "PostgreSQL 17.6 Documentation") |  F.27. pg_freespacemap — examine the free space map  
## Submit correction
If you see anything in the documentation that is not correct, does not match your experience with the particular feature or requires further clarification, please use [this form](https://www.postgresql.org/account/comments/new/17/pgcrypto.html/) to report a documentation issue. 
...
- Harvested: 2025-08-19T18:48:10.563347
- Profile: api_documentation
- Agent: architect
